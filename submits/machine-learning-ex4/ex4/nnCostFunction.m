...
a3 = reshape(sigmoid(z3), m ,num_labels);

J = (1/m)*sum(sum(-1.*y_i.*log(a3) - (1 - y_i).*log(1 - a3)));
...
%  y_i(y(i)) = 1;
%  
%  a2 = [ones(1, 1); sigmoid(X(i, :) * Theta1')'];
%  a3 = reshape(sigmoid(Theta2 * a2), 10, 1);
%  cumsm = sum((-1).*(y_i.*log(a3)) - (1 - y_i).*log(1 - a3));
%  J = J + cumsm;
